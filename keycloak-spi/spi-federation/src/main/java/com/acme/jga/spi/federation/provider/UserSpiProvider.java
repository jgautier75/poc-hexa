package com.acme.jga.spi.federation.provider;

import com.acme.jga.crypto.decode.CryptoDecoder;
import com.acme.jga.spi.federation.adapters.UserAdapter;
import com.acme.jga.spi.federation.constants.FederationConstants;
import com.acme.jga.spi.federation.dtos.UserInfosDto;
...
public class UserSpiProvider implements UserLookupProvider, UserStorageProvider, CredentialInputValidator {
    ...
    private Optional<UserInfosDto> fetchUser(FederationConstants.SearchFilterField searchField, String searchValue) {
        try {
            HttpClient httpClient = HttpClient.newBuilder().version(HttpClient.Version.HTTP_2).connectTimeout(Duration.ofSeconds(30)).build();
    ...
    private void initCredentials(RealmModel realm, UserInfosDto userInfosDto, UserModel userModel) {
        if (userInfosDto.getEncryptedPassword() != null) {
            String decodedPassword = cryptoDecoder.decode(userInfosDto.getEncryptedPassword());
