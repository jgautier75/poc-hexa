...
import com.acme.jga.spi.dao.extractors.SectorExtractor;
import com.acme.jga.spi.dao.utils.AbstractJdbcDaoSupport;
import com.acme.jga.spi.dao.utils.DaoConstants;
import com.acme.jga.spi.dao.utils.WhereClause;
import com.acme.jga.spi.dao.utils.WhereOperator;
...
public class SectorsDaoImpl extends AbstractJdbcDaoSupport implements com.acme.jga.spi.dao.sectors.api.SectorsDao {
    ...
    @Override
    public Integer deleteForOrganization(CompositeId tenantId, CompositeId organizationId) {
        String baseQuery = super.getQuery("sector_delete_by_org");
        List<WhereClause> whereClauses = new ArrayList<>();
        whereClauses.add(WhereClause.builder()
                .expression(buildSQLEqualsExpression(DaoConstants.FIELD_TENANT_ID, DaoConstants.P_TENANT_ID))
                .operator(WhereOperator.AND)
    ...
    public Integer deleteForTenant(CompositeId tenantId) {
        String baseQuery = super.getQuery("sector_delete_by_tenant");
        List<WhereClause> whereClauses = new ArrayList<>();
        whereClauses.add(WhereClause.builder()
                .expression(buildSQLEqualsExpression(DaoConstants.FIELD_TENANT_ID, DaoConstants.P_TENANT_ID))
                .operator(WhereOperator.AND)
