...
sed -e "s/MYIPADDRESS/${MYIP}/g" all_stack.yml > ./tmp/docker-services-base.yml

echo "Start 'base' docker services"
docker compose -f ./tmp/docker-services-base.yml --env-file app.env up -d
...
if [ "$STATUS" == "UP" ]; then
 docker exec -it $(docker ps --filter "label=com.acme.jga.poc.hexa=poc-hexa-keycloak" -q) sh -c /opt/keycloak/bin/setupFederation.sh
fi
