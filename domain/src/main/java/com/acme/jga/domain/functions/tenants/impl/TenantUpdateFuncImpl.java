...
public class TenantUpdateFuncImpl implements TenantUpdateInput {
    ...
    public boolean update(Tenant tenant) throws FunctionalException {
        boolean exists = tenantExistsFunc.existsByExternalId(tenant.tenantId().get());
        if (!exists) {
            throw new FunctionalException(Scope.TENANT.name(), FunctionalErrors.NOT_FOUND.name(), BundleFactory.getMessage("tenant.not_found", tenant.tenantId().get()));
        }
